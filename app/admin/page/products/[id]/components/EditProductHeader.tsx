...
export default function EditProductHeader({ isDarkMode, isArabic, onBack }: EditProductHeaderProps) {
  return (
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1">
              ✏️ {isArabic ? 'تعديل المنتج' : 'Edit Product'}
            </h1>
            <p className="text-sm sm:text-base text-blue-100">
              {isArabic ? 'تحديث معلومات المنتج' : 'Update product information'}
            </p>
          </div>
          <button 
            onClick={onBack} 
