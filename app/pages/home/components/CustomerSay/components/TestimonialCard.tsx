  ...
  return (
    <div
      className={`p-4 sm:p-5 md:p-6 rounded-xl shadow-sm border transition-all hover:shadow-md ${
        isDarkMode ? "bg-[#2A2D35] border-gray-700" : "bg-white border-gray-200"
      }`}
    >
      <div className="flex items-center mb-3 sm:mb-4">
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center mr-3 sm:mr-4 ${
          ...
          <span className={`text-base sm:text-lg font-bold ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
            {testimonial.name.charAt(0).toUpperCase()}
          </span>
        </div>
        <div>
          <p className={`font-semibold text-xs sm:text-sm ${isDarkMode ? "text-white" : "text-[#0D0D0D]"}`}>
            {testimonial.name}
          </p>
