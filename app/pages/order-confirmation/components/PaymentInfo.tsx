...
export default function PaymentInfo({ paymentMethod, orderId, isPrivateView, isDarkMode }: PaymentInfoProps) {
  return (
    <div className={`rounded-lg border p-6 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
      <h3 className={`text-lg font-semibold mb-4 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Payment Method</h3>
      ...
      <div className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
        <p>Transaction ID: {isPrivateView ? '••••••••' : orderId.slice(0, 16)}</p>
        <p className="mt-1">Payment Status: <span className="text-green-600 font-medium">Completed</span></p>
      </div>
    </div>
  );
}
