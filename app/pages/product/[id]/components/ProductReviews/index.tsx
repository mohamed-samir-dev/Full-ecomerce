...
export default function ProductReviews({ product }: ProductReviewsProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
      ...
      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-4 sm:mb-6">{isArabic ? 'آراء العملاء' : 'Customer Reviews'}</h3>

      {loading ? (
        <div className="text-center py-6 sm:py-8 text-sm sm:text-base text-gray-500">{isArabic ? 'جاري التحميل...' : 'Loading...'}</div>
      ) : reviews.length === 0 ? (
        <div className="text-center py-6 sm:py-8 text-sm sm:text-base text-gray-500">{isArabic ? 'لا توجد مراجعات' : 'There isn\'t one'}</div>
      ) : (
        <>
          <ReviewSummary 
