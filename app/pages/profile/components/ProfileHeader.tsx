...
export default function ProfileHeader({ user, isDarkMode, isArabic }: ProfileHeaderProps) {
  ...
  return (
    <div className={`rounded-xl sm:rounded-2xl border-2 p-3 sm:p-4 md:p-6 shadow-xl ${
      ...
      <div className={`flex flex-col sm:flex-row items-center sm:items-center gap-3 sm:gap-4 md:gap-6 ${isArabic ? 'sm:flex-row-reverse' : ''}`}>
        <div className={`relative p-3 sm:p-4 rounded-xl sm:rounded-2xl ${isDarkMode ? 'bg-slate-700' : 'bg-slate-100'}`}>
          ...
          {user.role === 'admin' && (
            <div className="absolute -top-1 -right-1 p-0.5 sm:p-1 bg-blue-500 rounded-full">
              <ShieldCheckIcon className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
            </div>
          )}
        </div>
        
        <div className={`flex-1 text-center sm:text-left ${isArabic ? 'sm:text-right' : ''}`}>
          <div className={`flex flex-col sm:flex-row items-center gap-2 sm:gap-3 mb-1 sm:mb-2 ${isArabic ? 'sm:flex-row-reverse' : ''}`}>
