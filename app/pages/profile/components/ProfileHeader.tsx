...
export default function ProfileHeader({ user, isDarkMode }: ProfileHeaderProps) {
  ...
  return (
    <div className={`rounded-xl sm:rounded-2xl border-2 p-3 sm:p-4 md:p-6 shadow-xl ${
      ...
      <div className={`flex flex-col sm:flex-row items-center gap-3 sm:gap-4 md:gap-6 ${isArabic ? 'sm:flex-row-reverse sm:justify-end' : ''}`}>
        ...
        <div className={`flex-1 ${isArabic ? 'flex flex-col items-end' : ''}`}>
          <div className={`flex flex-col ${isArabic ? 'items-end' : 'items-start'} gap-2 sm:gap-3 mb-1 sm:mb-2`}>
          ...
          <p className={`text-xs sm:text-sm mb-0.5 sm:mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'} ${isArabic ? 'text-right' : 'text-left'}`}>
            {user.email}
          </p>
          
          <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-500'} ${isArabic ? 'text-right' : 'text-left'}`}>
